...
impl Color {
    ...
    fn blend_two_channel(a: u8, b: u8, ratio: f32) -> u8 {
        (((1.0 - ratio) * (a as u16).pow(2) as f32) + (ratio * (b as u16).pow(2) as f32)).sqrt()
            as u8
    }
...
impl ColorRange {
    ...
    pub fn get_color(&self, value: f32) -> color::Fg<color::Rgb> {
        match &self.mid {
            Some(mid) => {
                color::Fg(Color::blend_three(&self.empty, mid, &self.full, value).as_term_rgb())
            }
...
pub fn make(
    ...
    format!(
        "{reset}{left_cap}{color}{full_chars}{reset}{empty_chars}{right_cap}",
